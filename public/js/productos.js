// Página /productos: trae la marca (nombre) desde /api/content para que la barra de
// navegación y el pie coincidan con el resto del sitio, y la lista de categorías +
// precios desde /api/products.
...
// El sitio no tiene logo (nav-brand es texto directo) - alcanza con el nombre.
...
function renderProductos(categories) {
  ...
  if (withProducts.length === 0) {
    ...
    empty.textContent = 'Todavía no hay precios cargados.';
...
async function loadPage() {
  ...
  document.title = `Precios y paquetes — ${content.site_name || ''}`;
  renderBrand(content);
  renderNavLabels(content);
  setText('productos-title', content.productos_heading || 'Precios y paquetes');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  loadPage().catch((err) => {
    console.error('No se pudo cargar la página de precios:', err);
  });
});
